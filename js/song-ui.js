(function (global) {
  ...
  const roundButton = (command, name, glyph) => `<button class="song-icon" data-command="${command}" aria-label="${name}" title="${name}">${icon(glyph)}</button>`;
  ...
  function renderPanel() {
    ...
    if (pane === 'pause') {
      content = title('UNE PAUSE DANS LE VOYAGE', 'Le ciel peut attendre.') +
        `<p class="song-dialog-sub">${escape(game.level.name)}</p><div class="song-pause-echoes">${echoes()}</div>
      ...
      content = title('LES TROIS ÎLES DU CHANT', 'L’archipel des voix') + '<div class="song-islands">' + global.LumenSong.ISLANDS.map((island, index) => {
        ...
        return `<button class="song-island" data-song-island="${index}" ${unlocked ? '' : 'disabled'} aria-label="${escape(island.name)}${unlocked ? '' : ', verrouillée'}">
    ...
    const focus = panel.querySelector('.song-primary, [data-song-island]:not(:disabled), input, button');
  ...
  function echoes() {
    return game.song.lights.map(echo => `<span class="song-echo ${echo.found ? 'found' : ''}" style="--echo-color:${echo.color}" title="${escape(echo.name)}${echo.found ? ', retrouvé' : ', à retrouver'}" aria-label="${escape(echo.name)}${echo.found ? ', retrouvé' : ', à retrouver'}"><i class="echo-ear echo-ear-left"></i><i class="echo-ear echo-ear-right"></i><i class="echo-face"><b></b><b></b></i></span>`).join('');
  }
  function sync() {
    if (!game) return;
  ...
  function update(dt) {
    ...
    const key = [song.index, song.count, game.levelStars, game.levelCoins, song.style, game.audio.muted].join(':');
    if (key !== lastHud) {
      lastHud = key;
      byId('song-island-number').textContent = String(song.index + 1).padStart(2, '0') + ' / 03';
      byId('song-island-name').textContent = game.level.name;
      ...
      byId('song-sound').setAttribute('aria-label', game.audio.muted ? 'Activer le son' : 'Couper le son');
      byId('song-sound').title = game.audio.muted ? 'Activer le son' : 'Couper le son';
    ...
    byId('song-goal').textContent = target ? `${target.name} attend quelque part${distance < 230 ? ', tout près.' : '.'}` : 'Le chœur est réuni. Le passage est ouvert.';
    ...
    byId('song-combo-label').textContent = song.combo >= 10 ? 'EN PLEIN ÉLAN' : 'LE FIL DU CHANT';
  ...
  function attach(instance, callbacks) {
    ...
    document.querySelectorAll('[data-touch]').forEach(button => {
      ...
      if (names[button.dataset.touch]) button.title = names[button.dataset.touch];
    });
    game.on('song-rescue', echo => helpers.toast(echo.name + ' chante avec toi.'));
    ...
    document.addEventListener('keydown', event => {
      ...
      const controls = [...byId('song-panel').querySelectorAll('button:not(:disabled), input, [tabindex="0"]')].filter(element => element.getClientRects().length);
    ...
    document.addEventListener('fullscreenchange', () => {
      ...
      byId('song-fullscreen').setAttribute('aria-label', document.fullscreenElement ? 'Quitter le plein écran' : 'Plein écran');
    });
