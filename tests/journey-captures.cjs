...
(async () => {
  ...
  try {
    ...
    const captures = [];
    for (const [fixture, profile] of Object.entries(profiles)) {
    ...
    if (before) {
      ...
      assert.deepEqual(conditions(captures), conditions(before.captures), 'Before and after conditions or fixed saved profiles differ.');
      ...
      if (data.campaign.some(key => !before.content.campaign.includes(key))) {
    ...
    fs.writeFileSync(path.join(output, 'manifest.json'), JSON.stringify({ phase,
      ...
      sourceDigest: digest(['index.html', 'js/ui.js', 'js/song-ui.js', 'js/levels.js'].map(file => fs.readFileSync(path.join(sourceRoot, file), 'utf8')).join('\n')),
      ...
      content: data, profiles, comparedWithBefore: !!before, captures }, null, 2) + '\n');
